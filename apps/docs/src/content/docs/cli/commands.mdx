...
`--compact` (machine-friendly text output) is supported on runtime platform commands except `check`, which outputs JSON by default instead. `--help` is available on every command and shows in-terminal examples, exit codes, and AI-agent usage tips.
