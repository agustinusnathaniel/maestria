...
description: Complete reference for all maestria commands - status, install, plugin, update, uninstall, check, configure, doctor, and setup - with flags, options, and examples.
---
...
`--compact` (machine-friendly text) is supported on status, install, update, uninstall, configure, and setup. `check` and `doctor` support `--json` and `--quiet` only; `plugin validate` and `plugin install` support `--json` only. Every command's `--help` shows exit codes. Examples appear on every command except `uninstall`; AI-agent tips appear on `maestria`, `configure`, `plugin`, `plugin install`, `plugin validate`, and `setup`.
...
<AllPackageManagers type="dlx" pkg="maestria" args="doctor" />
...
<AllPackageManagers type="dlx" pkg="maestria" args="setup --ecosystem codegraph,opensrc --yes" />
