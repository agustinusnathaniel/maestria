...
| ------------------ | ------- | ---------------------------------------------------------------------------------------------------------- |
| `id`               | string  | Platform identifier (`opencode`, `omp`, `pi`, `kimi-code`, `hermes`, `cursor`, `claude-code`, `codex-cli`) |
...
codex-cli: available installed=0.2.0 latest=0.2.0
```
...
| `codex-cli`   | Codex CLI   | npm package staged into a local Codex marketplace       |
...
| --------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Unknown platform ID         | `Unknown platform 'foo'. Valid platforms: opencode, omp, pi, kimi-code, hermes, cursor, claude-code, codex-cli` |
...
Exact version pinning is not available for `claude-code` or `codex-cli`; those host marketplace
...
| `maestria update --all --version 0.5.0`    | Updates pin-capable platforms to v0.5.0; Claude Code and Codex CLI return a version-pinning validation result                          |
