...
`--compact` (machine-friendly text output) is supported on runtime platform commands except `check`. `--help` is available on every command and shows in-terminal examples, exit codes, and AI-agent usage tips.
...
The version values in this example are illustrative. The command reports the versions found on your machine and the versions currently available from each registry.
...
- **Global scope only.** Maestria never scans, counts, or modifies project registrations. Each Prime command runs in a fresh, empty temporary directory so only user settings are visible; the directory is removed afterward.
- **Latest version only.** Prime's package manager accepts no version specifier and skips pinned registrations during updates. Maestria detects a pinned user-scope registration and reports an error before updating, rather than silently skipping it.
...
Validate and stage portable Agent Plugins v1 directory packages. This workflow is separate from the runtime platform registry: it prepares a package for a compatible client's own installer or directory loader.
...
The staged directory contains the root `plugin.json` and fixed `skills/` layout. The command does not activate the package in every client: installation scope, permissions, trust, and session behavior remain client-owned.
...
maestria plugin install @maestria/agent-plugin@0.1.2
...
Use `--destination` to choose another directory. The CLI refuses to overwrite an existing destination, so choose a new directory when staging a second copy.
...
The validator checks the closed `plugin.json` manifest, Agent Skills frontmatter and layout, optional `mcp.json` configuration, and filesystem containment. It does not execute package code or grant the client any permissions.
...
Exact version pinning is not available for `claude-code`, `codex`, `prime-agent`, or `hermes`. The first three update from the latest staged npm package and return a validation error if `--version` is passed. Hermes ignores the requested version, warns, and updates from its git-based plugin installation.
...
The cache stores only the last resolved version per package, with no time-to-live, so it is an offline fallback rather than a freshness window. To clear the fallback data:
...
A successful update removes that package's cached entry, and the next version check fetches live from npm.
...
Check the installation status of a maestria plugin on a specific platform, or on every detected platform with `--all`.
...
`<platform>` is one of `opencode`, `omp`, `pi`, `prime-agent`, `kimi-code`, `hermes`, `cursor`, `claude-code`, `codex`. Pass a platform or `--all`; the command errors when neither is given.
...
Unlike every other command, `check` has no `--compact` flag. The default output is human-readable text; pass `--json` for machine-readable output. `--quiet` suppresses error messages, not the result, so `--json --quiet` gives structured output without stderr noise.

### Behavior

`check` validates the platform, checks whether its CLI is available on `$PATH`, then checks whether the `@maestria/<platform>` plugin is installed. With `--all`, it runs the same check for every detected platform.
...
With `--all`, the exit code is `1` when any detected platform is unavailable or not installed, `3` when all detected platforms are installed and at least one is outdated, and `0` otherwise. Every JSON result includes the `platform` field with the checked platform id, and `latestVersion` is included only when it could be determined. JSON is written to stdout only when `--json` is passed; otherwise the default text output is written.
...
Unmentioned agents remain unchanged. Configuration currently supports OpenCode, Codex CLI, Cursor, Pi, and OMP.
