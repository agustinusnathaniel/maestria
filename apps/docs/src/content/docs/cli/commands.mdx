...
| `--quiet` | Suppress non-essential stderr output; the exit code is the signal        |

Unlike every other command, `check` has no `--compact` flag. For exit-code-only scripting, pass `--no-json` (or use `--quiet` and ignore the JSON output) - the exit code is the signal.
