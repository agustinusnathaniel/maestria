...
`--compact` (machine-friendly text) is supported on runtime platform commands except `check`. Every command's `--help` shows in-terminal examples, exit codes, and AI-agent usage tips.
...
Show what is installed and version information for all platforms.
...
Version values are illustrative; the command reports the versions found locally and available from each registry.
...
With `--compact`, status outputs one line per platform without colors or decoration:
...
**Runtime errors** - a failing platform command (network error, missing binary) is reported per-platform rather than aborting the batch:
...
Use `--destination` to choose another directory; the CLI refuses to overwrite an existing destination.
...
Same logic as `install`, but targeting platforms where maestria is already installed. Before-and-after version numbers are shown when available:
...
Like `install` and `update`, `uninstall` exits with code `1` in non-interactive terminals when no platform is passed and `--all` is not set, showing a clear error instead of prompting. JSON output is an array of per-platform results with `id`, `label`, `ok`, and `message` fields.
...
- Explicit model IDs come from the platform's live model list and are validated before writing.
