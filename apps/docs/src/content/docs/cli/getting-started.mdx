...
Portable commands return a validation report with `valid`, `name`, `version`, `root`, `skillNames`, `errors`, and `warnings`. `plugin install` also returns `destination` and `source`.
...
`plugin validate` exits with `1` when the package is invalid. `plugin install` exits with `1` when fetching, validation, or staging fails.
