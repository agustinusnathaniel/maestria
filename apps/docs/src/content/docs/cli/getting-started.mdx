...
The command validates the package and prints its local directory. Give that directory to the client's Agent Plugins installer or local-plugin setting. For a local package, run `maestria plugin validate /path/to/plugin` before staging it.
