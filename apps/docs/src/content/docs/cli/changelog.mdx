...
- Added `install`, `update`, and `uninstall` adapters that delegate to Prime's native package
  manager (`prime-agent package install`/`update`/`remove npm:@maestria/prime-agent`).
...
- Registration state is read from `prime-agent package list`, and the installed version is read
  from the installed package's manifest.
- Updates use Prime's latest-only package semantics, so exact version pinning is unavailable.
...
- The CLI stages the published npm packages under `~/.cache/maestria/` and delegates plugin state
  to each host's native marketplace manager.
