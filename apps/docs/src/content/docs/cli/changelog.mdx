...
- Prime support is deliberately global (user scope only): every Prime command runs from a freshly created empty temporary directory, so project registrations are never scanned or modified, and the temporary directory is removed afterwards.
