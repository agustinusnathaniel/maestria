---
title: 'maestria'
description: Manage maestria plugins across coding agent platforms from a single command - install, update, and check status across platforms, and configure per-agent models where supported.
template: splash
editUrl: false
lastUpdated: false
hero:
  title: 'One CLI to Manage Them All'
  tagline: 'Install, update, and check maestria across coding agent platforms. Configure per-agent models where the host runtime supports it.'
  actions:
    - text: Get Started
      icon: right-arrow
...
Maestria ships the same AI engineering methodology to multiple coding agent platforms. Each has its own install commands, update paths, and version management. `maestria` wraps them all behind a single interface, including Claude Code and Codex CLI.
...
| Codex CLI   | `maestria install codex-cli`                | `maestria update codex-cli`          |
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Full reference for status, install, update, and configure commands with flags."
    href="/cli/commands/"
  />
</CardGrid>
