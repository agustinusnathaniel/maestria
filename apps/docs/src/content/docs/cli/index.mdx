---
title: 'maestria'
description: Manage maestria plugins across all coding agent platforms from a single command - install, update, check status, and configure per-agent models for OpenCode, Kimi Code, and Pi.
template: splash
editUrl: false
lastUpdated: false
hero:
  title: 'One CLI to Manage Them All'
  tagline: 'Install, update, check the status, and configure per-agent models for maestria plugins across OpenCode, Kimi Code, and Pi - from a single command.'
  actions:
    - text: Get Started
      icon: right-arrow
...
1. **Run <AllPackageManagers type="dlx" pkg="maestria" />** - no install required, thanks to the bundled single-file `.mjs` distribution
