...
> **Status: Native candidate.** The package validates with the official Claude Code CLI, but runtime behavior is not yet tested end to end; see the [runtime support matrix](https://github.com/agustinusnathaniel/maestria/blob/main/docs/runtime-support-matrix.md).
...
The CLI downloads `@maestria/claude-code` from npm, stages it in a local marketplace under `~/.cache/maestria/`, and delegates the actual install to Claude Code. Plugin state and scope stay in the host runtime.
