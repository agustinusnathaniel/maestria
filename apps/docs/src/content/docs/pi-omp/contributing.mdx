...
**Canonical skills** (orchestrator, global-rules, handoff, iteration-limits) - these are synced from core via `sync.config.ts`. Edit the canonical source, then run `scripts/sync-all` from the repository root.

**Platform-only additions** - if a new skill is specific to one package, keep it in that package's `skills/` directory. Give it a valid `SKILL.md` with `name` and `description`, then validate with:
