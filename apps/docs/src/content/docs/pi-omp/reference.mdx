...
<Aside type="note">
  **Pi** restricts the orchestrator to only `maestria_subagent` during enforcement. **OMP** allows
  ...
  plus OMP's native `goal` tool while native goal mode is active. The `maestria_subagent` tool
  validates parameters and constructs a structured handoff, then returns a plan for the native
  `task()` call. Subagent sessions (without the `task` tool) are not restricted and retain their
  full specialist toolset.
</Aside>
