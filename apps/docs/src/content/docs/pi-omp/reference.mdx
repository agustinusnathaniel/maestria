...
The orchestrator gives each specialist a **compact, material handoff** carrying only what is needed to act: outcome, relevant context and constraints, acceptance or expected evidence, material assumptions or known problems, and the next step or blocker. There is no fixed field count; the orchestrator validates handoffs before dispatch and rejects incomplete ones.
...
**Pipeline discipline.** For non-trivial work, the flow is adventurer (recon) → architect or planner (design/plan) → builder (implement) → reviewer (validate); each step verifies before handing off. Skipping recon or review trades speed for risk.

## Session Tree Integration

Every subagent dispatch records the parent task ID and specialist name in the session tree. Compaction preserves this state so resumed or forked sessions retain full context. The session tree is surfaced via `/maestria-status` and compaction summaries. Mode and delegation state also survive session compaction, resume, and fork.
...
When a workflow mode is active (fein/sonar/blitz), the platform may restrict the main session to delegation tools. This can enforce the maker/checker split at the tool level, and the exact restriction differs between Pi and OMP:

- **Pi** restricts the orchestrator to only `maestria_subagent` during enforcement.
- **OMP** allows both `task()` (native) and `maestria_subagent` (structured handoff wrapper) during enforcement.

The `maestria_subagent` tool validates parameters and constructs a structured handoff, then returns a plan for the native `task()` call. Subagent sessions (without the `task` tool) are not restricted and retain their full specialist toolset.

### Enforcement lifecycle

The orchestrator prompt and global rules are always active. The tool-level restriction (blocking `bash`/`edit`/`write` etc.) is **not active by default**; it activates only when you set a workflow mode.
...
OMP's `maestria_subagent` tool validates the request, records the handoff, and returns a plan for OMP's native `task()` tool, which performs the actual dispatch. The wrapper is a structured planning layer, not an equivalent implementation of Pi's dispatch lifecycle.
...
| Agent | Tools | Purpose |
| --- | --- | --- |
| adventurer | read, bash, grep, find, ls, glob | Codebase reconnaissance; no write/edit tools |
...
| planner | read, bash, grep, find, ls | Planning; no write/edit tools |
| reviewer | read, bash, grep, find, ls, glob | Code review; no write/edit tools |
