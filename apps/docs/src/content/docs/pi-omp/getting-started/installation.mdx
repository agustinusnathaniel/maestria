...
`@maestria/pi` and `@maestria/omp` are extensions that turn the [Pi](https://pi.software) or [Oh My Pi](https://omp.sh) coding agents into disciplined engineering workstations. Each registers 7 specialist subagents, 3 workflow mode commands, and spec-driven orchestration - all with a single extension entry.
...
<Aside type="tip">
  To pin a specific version for Pi, use `pi install npm:@maestria/pi@0.7.5`. Check the
...
<Aside type="tip">
  To pin a specific version for OMP, use `omp install @maestria/omp@0.5.5`. Check the
  [changelog](/pi-omp/changelog/) for version history.
</Aside>
