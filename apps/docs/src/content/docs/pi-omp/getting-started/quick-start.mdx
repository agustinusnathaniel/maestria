...
- **`/sonar`** - Research only: read-only reconnaissance/planning → STOP. No implementation. Use for discovery and feasibility.
- **`/blitz`** - Fast implementation: skip optional recon/design ceremony, but required validation and review still apply. Use for quick fixes in known territory.
...
<Aside type="note">
  Without a mode keyword, the agent operates with the orchestrator prompt as guidance but without
  tool-level enforcement. On Pi/OMP, mode state persists for the process after `/fein`, `/sonar`, or
  `/blitz`; use `/mode-clear` to return to neutral routing. The dispatcher restriction activates
  only when a mode is active.
</Aside>

## How Dispatch Works

Pi dispatches specialists through `maestria_subagent(agent, task)` from the `@gotgenes/pi-subagents` extension. OMP uses its built-in `task(agent: "...", task: "...")` tool and needs no external dispatch dependency.

Their limits, context inheritance, and enforcement differ. See the [Pi and OMP reference](/pi-omp/reference/) before relying on parallel or chained dispatch.

When a workflow mode activates enforcement, the current platform can restrict the main session to delegation tools. Mode-free and specialist sessions keep different tool access.

## Your First Task

For a task that warrants the full pipeline:
...
In Pi and OMP, the main session applies the orchestrator guidance and dispatches specialists. There is no separate agent to switch to: activate a workflow mode to select the route and its enforcement.
