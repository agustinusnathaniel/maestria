...
> **Note:** Without a mode keyword, the agent operates with the orchestrator prompt as guidance but without tool-level enforcement. On Pi/OMP, mode state persists for the process after `/fein`, `/sonar`, or `/blitz`; use `/mode-clear` to return to neutral routing. The dispatcher restriction activates only when a mode is active.
