...
The orchestrator is the methodology layer. It does not implement, debug, or edit code itself — it loads specialist skills via the `Skill` tool and dispatches them to Kimi Code's 3 built-in subagents via `Agent` (1–2 items) or `AgentSwarm` (≥3 uniform items).
