...
<Aside type="caution" title="The orchestrator is not always the fastest path">
  Kimi Code can dispatch several subagents, but every dispatch adds model work and waiting time. Use
  [the usage decision guide](/core/when-to-use/) before routing a small task or an expensive model
...
The full pipeline diagram is in [Pipeline & Roles](/core/pipeline/).

<Aside type="tip">
  Choose the smallest safe route. Use direct execution for known, low-risk work when the host
  permits it; use recon (`adventurer`) or design (`architect`) when the code is unfamiliar or the
  approach is uncertain. The orchestrator handles this routing automatically - but you can also
