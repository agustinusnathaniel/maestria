...
> **Tip:** Pair `/fein` with `/goal` for multi-session work. Set `/goal draft Run the fein pipeline to implement OAuth` - the goal persists across sessions and restores your mode on `/goal resume`. See [Goals docs](https://hermes-agent.nousresearch.com/docs/user-guide/features/goals).
