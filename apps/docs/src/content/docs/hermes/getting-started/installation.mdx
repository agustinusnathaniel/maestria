...
description: How to install @maestria/hermes on your Hermes Agent and what you get - 9 methodology skills plus 3 command skills, 7 slash commands, and role-neutral child trust boundaries.
---

import { Steps, Aside, Tabs, TabItem, Code } from '@astrojs/starlight/components';
import { PackageManagers } from 'starlight-package-managers';

`@maestria/hermes` is a git-based Hermes plugin that turns your Hermes Agent into a disciplined methodology worker. It registers 9 methodology skills plus 3 command skills, 7 slash commands, role-neutral child trust, and lifecycle hooks - all installed with a single `hermes plugins install` command.
...
| Resource                              | What                                                                                                                                                                                                                                                                                                                           |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 12 skills (9 methodology + 3 command) | 9 methodology skills (7 specialists + orchestrator + global rules) plus 3 command workflow skills (`/fein`, `/sonar`, `/blitz`)                                                                                                                                                                                                |
| 7 commands                            | `/fein`, `/sonar`, `/blitz`, `/mode`, `/mode-clear`, `/review`, `/plan`                                                                                                                                                                                                                                                        |
| Lifecycle trust                       | Trust comes from effective native topology roles (`leaf`/`orchestrator`) and recognized top-level bindings; Maestria specialist names are not trust identities. Delegated children get a fixed read/research/LLM-only policy (role-neutral child trust); no child role grants write/execute/shell/delegate/OpenCode capability |
| Hooks                                 | `pre_gateway_dispatch`, `pre_llm_call`, `pre_tool_call`, `on_session_start`, `on_session_end`, `on_session_finalize`, `on_session_reset`, `subagent_start`, `subagent_stop`, `transform_tool_result`                                                                                                                           |
