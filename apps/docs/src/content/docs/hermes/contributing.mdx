...
1. Add the handler in `src/maestria_hermes/__init__.py` or a focused module under `src/maestria_hermes/`.
...
1. Create `src/maestria_hermes/hooks/<name>.py` with the handler function
2. Wire it in `__init__.py`'s `register()` via `ctx.register_hook()`
3. Add the hook name to `plugin.yaml`'s `provides_hooks` list

### Adding a skill

1. Create the `SKILL.md` file in `src/maestria_hermes/skills/<name>/`
