...
- Project customization (`.maestria/workflow.md` then `.maestria/rules.md`) loads as subordinate guidance, re-read every `pre_llm_call` turn with a visible error banner; see the [methodology changelog](/core/changelog/).
...
- Removed dead role and permission scaffolding that ADR-HM-002 already recorded as removed, unified slash-command mode text, and restored the fail-closed denial for unknown mode values; generated skill guidance no longer describes the removed role model.
