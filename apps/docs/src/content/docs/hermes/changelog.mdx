...
> **Supersession note (current, 2026-08-10):** The "Role-based permission enforcement" item below describes per-specialist tool restrictions driven by `[MAESTRIA_ROLE: <specialist>]` markers in `delegate_task` context. That role-based child-permission model is **superseded** by the approved role-neutral child trust policy (ADR-HM-002, 2026-08-10): delegated children receive a fixed read/research/LLM-only policy, and `[MAESTRIA_ROLE: ...]` markers are ignored. The historical entry is preserved unchanged as a record of what shipped in 0.1.3. The `pre_gateway_dispatch` and `transform_tool_result` entries below remain current - both hooks are still registered in the runtime.
