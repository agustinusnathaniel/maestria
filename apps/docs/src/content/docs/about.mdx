...
Maestria is portable AI engineering praxis: a set of free, open-source plugins that install a complete working methodology into AI coding agents, instead of asking you to memorize one.

Native integrations use `npx maestria install <platform>` and give an agent a dispatch-only orchestrator supported by seven specialists - adventurer (reconnaissance), architect (approach and trade-offs), builder (atomic implementation), diagnose (root-cause analysis), planner (phased plans), reviewer (independent review), and writer (documentation). For clients that support Agent Plugins v1, [`@maestria/agent-plugin`](/agent-plugin/) provides the methodology as portable skills, and `npx maestria plugin install` stages the package for the client's own installer.

The same methodology then runs natively on OpenCode, Kimi Code, Pi, Hermes, Claude Code, Codex CLI, Cursor, prime-agent, and OMP.
