...
> Orchestrator: Review this authentication change generally, with a security lens for injection risks, auth bypasses, and data exposure.
> Orchestrator: Review this dashboard change generally, with a UX lens for accessibility, interaction patterns, and responsive states.
...
Use **Parallel speculation** only when the right approach is genuinely unclear
and multiple independent perspectives would materially improve the decision.
...
The orchestrator dispatches the question to independent specialists with
...
For a clear, low-risk task, use one route and let the assigned specialist or
host agent reason and act without unnecessary fan-out.
...
Runtime authority, selected-route safeguards, maker/checker review where applicable, bounded repair,
and the commit protocol always take precedence over project instructions. If a conflict arises, the
core principle wins.
