...
| **Builder**      | Focused implementation — one atomic task per invocation                                                     | Bug fixes, feature work, tests, refactors — after reconnaissance and design are complete             | Full edit access; requires explicit approval for shell commands        |
...
| **Reviewer**     | Code review — correctness, edge cases, security, performance, maintainability                               | Pre-merge review, security audit, post-implementation validation                                     | Read-only — cannot edit; uses structured review format                 |
