...
- **PR and docs workflows are standalone skills.** The `create-pull-request` and `docs-update` skills carry their procedures, while the core keeps a short obligation plus a pointer; see the [CLI changelog](/cli/changelog/) for install and selection behavior.
