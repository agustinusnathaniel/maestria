...
Define a verifiable stopping condition before delegating. A repair loop normally gets about three attempts, extended only when the latest attempt shows new progress; if the same cause recurs, escalate with:
