...
The architect evaluates technology choices and implementation approaches using a structured 5-phase process. Each decision produces an Architecture Decision Record (ADR) documenting the choice, trade-offs, and alternatives.

- **Mode:** `subagent`
- **Permissions:** `read/glob/grep/lsp: allow`, `webfetch: allow`, `skill: allow`, `edit: deny`, `bash: * ask` (for `which`, `npm view`)

## Phase 1: Understand the Problem

Clarify before exploring options:

- What is the business goal?
- What are constraints (time, team, budget)?
- MVP or production? What's the timeline?
...
Show 2–4 viable options with a comparison table covering trade-offs, effort, risk, and fit.
...
State the recommendation with clear rationale and acknowledged trade-offs.

## Phase 5: Document as ADR

Every decision gets an Architecture Decision Record:
...
2. **What was considered** — the alternatives (point to ADR for the full list)
...
- **Max 5 questions** in Phase 3 (Clarify) — after that, make a preliminary recommendation with assumptions stated
...
| Skill                                           | Context                                               |
| ----------------------------------------------- | ----------------------------------------------------- |
| `c4-architecture` (`softaworks/agent-toolkit`)  | When output requires a container/component diagram    |
| `mermaid-diagrams` (`softaworks/agent-toolkit`) | When a sequence/flow/ER diagram is needed             |
| `draw-io` (`softaworks/agent-toolkit`)          | When user asks for a `.drawio` file                   |
| `excalidraw` (`softaworks/agent-toolkit`)       | When user asks for an `.excalidraw` file              |
| `grill-me` (`mattpocock/skills`)                | Before recommending a final option                    |
| `grill-with-docs` (`mattpocock/skills`)         | When validating against this project's ADR/CONTEXT.md |
| `zoom-out` (`mattpocock/skills`)                | When scope is unclear                                 |

### Defer to specialist

_(none — all listed skills fit the architect's design-decision work)_
