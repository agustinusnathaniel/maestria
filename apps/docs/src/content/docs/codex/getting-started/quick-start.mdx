...
Once `npx maestria install codex` completes, start a fresh Codex CLI session so the plugin
marketplace state and managed global orchestration instructions are loaded. Normal engineering
requests now start in the host-owned Codex session, which applies the Maestria orchestrator
...
The orchestrator chooses between direct work and native specialist agents based on task risk and
available runtime capabilities. It is methodology guidance, not a replacement for Codex's own
approvals or sandbox.
...
When the native companion pack is installed, the same roles are available to Codex delegation as
`maestria-adventurer`, `maestria-architect`, `maestria-builder`, `maestria-diagnose`,
`maestria-planner`, `maestria-reviewer`, and `maestria-writer` through the native `agent_type`
selector.
