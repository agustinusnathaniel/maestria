...
CLI installs the skills through Codex's plugin manager and the agent TOMLs through Codex's native
`$CODEX_HOME/agents/` directory. It does not write `AGENTS.md`, register models, add MCP servers,
or install hooks.
...
`maestria-*.toml` agent roles under `$CODEX_HOME/agents/`.
