...
- The CLI adds a marked global instruction block so Codex's host-owned primary session activates
  Maestria orchestration and delegates with native `agent_type` role names.
- Updates preserve user model, reasoning, and service-tier settings, and uninstall removes only
  Maestria-managed agent files and instruction content.
...
Initial Codex CLI projection. It packages the canonical Maestria specialist,
