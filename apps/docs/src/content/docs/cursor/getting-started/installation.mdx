---
title: Installation & Setup
description: Install @maestria/cursor for Cursor IDE and Cursor CLI. Declarative plugin - no build step.
---

import { Steps, Aside } from '@astrojs/starlight/components';
import { PackageManagers } from 'starlight-package-managers';

`@maestria/cursor` is a declarative Cursor plugin: a manifest, agents, an orchestrator skill, global rules, and slash commands. There is no `npm install` of a runtime SDK for end users.

## Prerequisites

- **Cursor IDE** and/or **Cursor CLI** (`agent` on `$PATH`)
- No Node toolchain required for end-user install

## Installation via CLI (Recommended)

<PackageManagers type="dlx" pkg="maestria@latest" args="install cursor" />
<PackageManagers type="dlx" pkg="maestria@latest" args="status" />

The CLI pulls `@maestria/cursor` from npm (`npm pack @maestria/cursor@latest`) and extracts it into `~/.cursor/plugins/local/maestria`.

### Updating

<PackageManagers type="dlx" pkg="maestria@latest" args="update cursor" />
<PackageManagers type="dlx" pkg="maestria@latest" args="status" />

To pin to a specific version:

<PackageManagers type="dlx" pkg="maestria@latest" args="update cursor --version 0.2.5" />

<Aside type="tip" title="Restart required">
  Restart Cursor IDE after install so Customize → Plugins picks up the local plugin. For CLI, pass
  `--plugin-dir` or rely on the local plugins directory.
</Aside>

<details>
<summary>Alternative: Manual setup from a monorepo checkout</summary>

<Steps>

1. **Copy the plugin**

   ```bash
   mkdir -p ~/.cursor/plugins/local/maestria
   cp -a packages/cursor/. ~/.cursor/plugins/local/maestria/
   ```

2. **Or run without installing**

   ```bash
   agent --plugin-dir ./packages/cursor
   ```

3. **Verify**

   In Agent chat, confirm `/fein`, `/sonar`, `/blitz`, `/orchestrator` appear, and that specialist agents (builder, reviewer, and others) are available.

</Steps>

</details>

### CLI Agent Invocation

When running Cursor CLI (`agent`) with the plugin:

```bash
agent --plugin-dir ~/.cursor/plugins/local/maestria
```

For non-interactive usage (CI, scripts):

```bash
agent --plugin-dir ~/.cursor/plugins/local/maestria --print "your prompt"
```

## What gets installed

| Path                           | Purpose                  |
| ------------------------------ | ------------------------ |
| `.cursor-plugin/plugin.json`   | Plugin manifest          |
| `agents/*.md`                  | 7 specialist Task agents |
| `skills/orchestrator/SKILL.md` | Dispatcher methodology   |
| `rules/maestria-global.mdc`    | Always-on global rules   |
| `commands/*.md`                | Workflow slash commands  |

## Uninstall

<PackageManagers type="dlx" pkg="maestria@latest" args="uninstall cursor" />

## Next steps

Continue to the [Quick Start](/cursor/getting-started/quick-start/) to run your first pipeline.
