    ...
    >
      <FeatureGrid
        items={[
          ...
          { title: 'Always-on global rules', desc: 'rules/maestria-global.mdc ships with alwaysApply: true, so rules apply to every Agent session.' },
          ...
          { title: 'Declarative plugin', desc: '.cursor-plugin/plugin.json declares rules/, agents/, skills/, and commands/ for Cursor to auto-load.' },
        ]}
      />
    </PluginSection>
