...
- ADR-CORE-017 (selective effect v4 adoption)
- ADR-CORE-018 (documentation standard)
- ADR-CORE-019 (directive simplification)
- ADR-CORE-020 (hybrid package topology)
- ADR-CORE-021 (ultracite via vite-plus)
- ADR-CORE-022 (agent plugins portable projection)
- ADR-CORE-023 (evidence-led directives)
- ADR-CORE-024 (anchor liveness preflight)
- ADR-CORE-025 (consumer-driven sync and adapter simplification)
- ADR-OC-005 (orchestrator mcp permissions)
- ADR-HM-000 (plugin over skills-only)
- ADR-HM-001 (long-lived goals scope)
- ADR-HM-002 (orchestration policy)
- ADR-CR-001 (cursor plugin architecture)
