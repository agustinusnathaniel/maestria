...
8. Validate docs-related changes (`apps/docs/**` and `docs/**`) through the docs-site deployment build. The docs site is built and link-checked by every deployment preview (Cloudflare Pages runs the Starlight build, including `starlight-links-validator`), covering the docs-only changes that `ci.yml`'s `paths-ignore` skips.

## Non-Goals

- No changes to package source code, sync pipeline, generated projections, lockfiles, or existing CI workflows. No dedicated GitHub Actions docs-site workflow is added: deployment previews are the docs validation path (see Decision), and existing workflow files (`ci.yml` and its path filters) are left unchanged.
...
- No programmatic enforcement of the documentation format (no new lint rules or format-validation CI checks). The docs-site deployment build (see Decision) validates that the site builds and links resolve; it is not a format gate.
...
8. **Rely on deployment preview builds for docs validation** instead of a dedicated GitHub Actions workflow. The docs site deploys to Cloudflare Pages - production on `main` pushes, a preview deployment on every PR (the Pages GitHub integration is not path-filtered, so docs-only PRs - which `ci.yml`'s `paths-ignore` skips - get a preview build too). The deployment build runs `pnpm --filter @maestria/docs build` (see `apps/docs/netlify.toml`), which performs the Starlight build including `starlight-links-validator` link checking; `astro build` runs content-collection sync internally, so the separate `pnpm sync:docs` step is a type-generation convenience, not a required validation step. No separate `.github/workflows/docs.yml` is maintained: deployment preview is the docs validation path, and no project rule requires a GitHub workflow for docs. The build also runs for `docs/**`-only changes even though root docs do not affect the site build; that cost is accepted because deployment previews build on every push.
...
- Docs-related changes (`apps/docs/**` and `docs/**`) are validated by the docs-site deployment build on every preview and production deploy, instead of silently bypassing CI; root `ci.yml` path filters are untouched and no separate docs workflow is maintained.
...
- Docs validation depends on the Cloudflare Pages deployment pipeline: the Pages dashboard build command must keep running the docs build (`pnpm --filter @maestria/docs build` per `apps/docs/netlify.toml`), and every PR/push incurs a preview build even for `docs/**`-only changes that do not affect the site build (accepted because deployment previews build on every push and a docs-only PR would otherwise run no checks at all).
