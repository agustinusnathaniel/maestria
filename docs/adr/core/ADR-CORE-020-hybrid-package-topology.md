...
2. **Skill-validator consolidation.** Move the common filesystem/frontmatter validation routine (missing file, `---` frontmatter guard, `name:`/`description:` presence, empty-body guard) to a reusable core script helper under `packages/core/scripts/lib/`. Keep `packages/pi/scripts/validate-skills.ts` and `packages/omp/scripts/validate-skills.ts` as thin platform wrappers that supply their skill lists and delegate, preserving output, exit status, and current validation strictness. Do not silently change Prime's stricter validator (`packages/prime-agent/scripts/skill-validation.ts`) - its name grammar (`^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$`, length 1-64, no `--`), length caps (`NAME_MAX=64`, `DESCRIPTION_MAX=1024`), and quoted-scalar normalization remain distinct.
...
- Pi and OMP validators now share a file under `packages/core/scripts/lib/` - a host-specific leniency change must be modeled as an option on the helper, not as a wrapper fork, or the helper's strictness would drift.
...
- Core script helper is under `packages/core/scripts/lib/` (preferred per task) to avoid a new top-level package for a build-time script. Pi/OMP wrappers preserve existing skill lists (`orchestrator, global-rules, handoff, iteration-limits`) and `✅`/`❌` output with correct exit codes; Prime's validator stays stricter and separate.
