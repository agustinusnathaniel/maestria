...
| --- | --- | --- |
| Orchestrator (now narrowed) | global rules (`packages/opencode/rules/AGENTS.md`) - "Webfetch may hang - don't block on it" directive | Structured paragraph + `opensrc` for repos, `webfetch` for pages, fallback if webfetch hangs - moved to global rules when the orchestrator's read-side tools were stripped in 2026-06 |
