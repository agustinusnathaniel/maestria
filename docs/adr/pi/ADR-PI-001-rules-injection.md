...
3. **Dynamic mode prompts:** The `before_agent_start` event is still used, but only for workflow mode prompt injection (`/fein`, `/sonar`, `/blitz`). Explicit slash commands persist their selection. A keyword in ordinary input is a one-turn override and is cleared by the next unmarked input; it does not alter persisted state. When no mode is active, the handler returns void (no modification).
...
`src/rules.ts:createModePromptHandler()` injects dynamic mode prompts via `before_agent_start` only when a workflow mode is active (no static content injection). `src/modes.ts` keeps automatic keyword detection transient while slash commands continue to persist state.
