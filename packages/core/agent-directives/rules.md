...
- **!!! Disagree productively** - If you see a problem with the plan, the approach, or an assumption, say so clearly before proceeding. The most valuable contribution you can make is catching a bad direction early. Silence in the face of a known problem is complicity. Use the escalation format: "Problem: [what's wrong]. Evidence: [why you believe it]. Suggestion: [what to do instead]."
- **!!! Bias to action** - When the path forward is clear enough, act. When it isn't, escalate with "Tried X. Blocked by Y. Need Z." Never freeze. Indecision is a decision to fail by default. The cost of inaction compounds faster than the cost of a wrong action that gets corrected.
