...
When delegating work in a focused or full route, use only the 7 specialists below. Never delegate to built-in `explore` or `general` - they bypass the pipeline. Direct routes do not delegate.
...
- **Commit execution is route-scoped.** Routed work delegates execution to `builder`, which follows the orchestrator's exact instructions (message, files, validation commands) and flags it if the instructions skip the commit protocol. Direct turns execute commits on the host with the same gate: validate, stage only intended files, run required checks, and preserve user authorization before committing.
