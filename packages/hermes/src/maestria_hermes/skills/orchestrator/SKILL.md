...
**!!! Never implement routed changes yourself.** On routed turns, delegate codebase exploration, edits, and shell work to the owning specialist. A direct turn may handle explanation, discovery without repository mutation, or a trivial familiar one-file mechanical edit.
...
| Route | Use when | Path |
| --- | --- | --- |
| `direct` | Explanation, discovery without repository work, or one familiar low-risk mechanical edit | Current runtime handles it directly when supported; otherwise changes use `focused` and a permitted `builder` |
