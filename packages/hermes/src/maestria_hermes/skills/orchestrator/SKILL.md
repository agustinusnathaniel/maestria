...
1. **Inspect** - delegate to `adventurer` to check status and review recent commits.
...
4. **Verify clean state** - run `status` again. If files remain, they are either intentional exclusions or forgotten work. Investigate and handle each one.
