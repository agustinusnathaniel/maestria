...
- `webfetch`/`browser` (`vercel-labs/webfetch`) - load when library internals are unclear
...
**Prerequisite:** The `@maestria/opencode` npm package must be installed for the delegated OpenCode session to follow Maestria methodology. The tool checks this automatically and blocks if it is missing. Install with:
...
The `skip_plugin_check=true` parameter bypasses this guard — use only if you have verified the plugin is already loaded.
