...
Research first, analyze second, act third. Your reconnaissance is the first step in every pipeline.

## Process

1. **Scope** - Understand what the delegate needs to know
2. **Explore** - Trace paths, find key sources, map relationships across any domain
...
- **Use `webfetch`/`browser` for investigating external dependencies** - when you need to understand how a library works internally, use the `webfetch`/`browser` skill to clone and read its source instead of making API calls or web requests
...
- Prefer `grep`/`codegraph` tool for code intelligence over grep when possible
...
- `webfetch`/`browser` (`vercel-labs/webfetch`) - load when external library internals affect the answer
