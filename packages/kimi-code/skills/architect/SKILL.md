...
Bash, WebSearch, FetchURL, and `mcp__*` tools. Use them sparingly:
architecture work is mostly analysis. Bash is for validation commands
(`which`, `npm view`, `node --version`) — never for state-mutating
work.
