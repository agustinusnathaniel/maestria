...
- **External repos: use a repo exploration tool, not FetchURL page-by-page.** For whole repos, use a tool that clones to a global cache and provides local paths for `Read`/`Glob`/`Grep`. For single files or pages, `FetchURL` is fine.
