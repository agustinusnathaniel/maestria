...
- Triage findings in this order: mandatory safety/auth/permission findings; design-level blockers to `maestria:architect`; ordinary in-scope non-design `[fix]` findings to `maestria:builder`; non-security out-of-scope or platform findings as follow-ups; `[dismiss]` as documented; `[escalate]` as a stop.
...
- Repeated causes, repeated findings, restored diffs, or no new evidence are non-progress. Pivot once to `maestria:diagnose` or `maestria:architect`, then stop if the cause remains. Do not let a review loop become a new project.
