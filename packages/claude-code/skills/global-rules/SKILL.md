...
- A work unit is one user outcome, its acceptance criteria, and its explicit non-goals. Before implementation or the first delegation, preserve/update the primary outcome, explicit non-goals, and termination condition; track route, owner, and finite budgets internally, without requiring a user-visible numeric ledger for default routes. Restate the outcome and non-goals at every material checkpoint.
...
- Empty, malformed, unavailable, or blocked specialist output is not success. Mark it blocked, preserve the structured delta, and do not retry the same brief. A delegation that is unavailable, malformed, or times out is not idle time: preserve the work ledger and artifacts, record it as terminal `blocked` or `failed` first, then dispatch at most one recovery attempt for the same child - a materially corrected brief when the cause is identifiable, otherwise one bounded transport retry. Recovery is a new attempt for the same child, not dependent work, and counts against the child's single recovery allowance shared with the changed-brief rule. Allow at most one changed-brief recovery per child when new evidence justifies it; a second empty or blocked result trips the task circuit breaker and escalates to `maestria:diagnose`, `maestria:architect`, or the user as applicable. Intentional user or platform cancellation is terminal `cancelled` and is never retried or continued. If recovery fails, preserve the terminal delta and stop dependent work; while the child is unavailable, continue independent read-only exploration, planning, and result reporting where useful. Never mutate code directly as a fallback and never waive review or safety floors.
...
- Before compaction or context rollover, preserve/update work-unit state: the record, acceptance condition, assumptions/evidence, child statuses and remaining budgets, last diff, verification/findings, process cleanup evidence, and next step. Resume from that preserved state; if it cannot be preserved, stop with a blocked handoff.
...
- Design-level blockers (requirements, public contract, data model, module boundary, threat model, or cross-cutting behavior) route to `maestria:architect` regardless of action label, not to repeated builder patches.
...
- Repeating a failure cause or review finding, restoring the same diff, or producing no new evidence is non-progress. Pivot once, then escalate: do not repeat the strategy - route root-cause uncertainty to `maestria:diagnose` and design uncertainty to `maestria:architect`. After one strategy pivot without progress, stop and escalate to an architecture decision.
