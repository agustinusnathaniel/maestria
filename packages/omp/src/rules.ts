 ...
 * void (no modification), letting the platform's prompt assembly (skills +
 * context files + tools) stand as-is.
 *
 * Fail-loud path: the OMP host swallows `before_agent_start` exceptions
 * (verified against pinned `@oh-my-pi/pi-coding-agent` 17.4.0:
 * `#runHandlerWithTimeout` catches handler errors into `emitError` and
 * continues), so this handler never throws. A present-but-unusable project
 * file surfaces via `ctx.ui.notify` plus a STOP banner appended to the
 * returned `systemPrompt` array instead of running with silently absent
 * config.
 */
  ...
  (
    ...
    if (root !== undefined) {
      try {
        ...
        return { systemPrompt: [...event.systemPrompt, '', formatProjectErrorBanner(message)] };
      }
    }
