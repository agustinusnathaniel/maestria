...
- Repeating a failure cause or review finding, restoring the same diff, or producing no new evidence is non-progress. Do not repeat the strategy: route root-cause uncertainty to `diagnose` and design uncertainty to `architect`. After one strategy pivot without progress, stop and escalate.
...
- Ordinary reviewer `[fix]` findings may be repaired automatically within the bounded budget, followed by validation and a blind re-review. Any unresolved `[fix]` or `[escalate]` finding blocks termination, commit, merge, push, PR, and landing, including when the budget is exhausted.
