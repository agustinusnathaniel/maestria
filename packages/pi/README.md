...
- **Mode Scope** - Slash-command selections persist. A mode keyword in ordinary input applies for that turn only; fein and sonar stay dispatcher-only, sonar is read-only, and blitz blocks specialist dispatch.
...
| `/blitz <goal>` | Set workflow mode to direct execution; independent review before shipping if an artifact lands |
