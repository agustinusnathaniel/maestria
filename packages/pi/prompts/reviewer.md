...
3. **Issues by severity** (with line references and concrete fixes). Prefix each issue with a Conventional Comments label: `praise:`, `suggestion:`, `issue:`, `nitpick:`, `question:`
4. **What was verified** (tests, edge cases, security checks) — **What was NOT verified** — out-of-scope, can't reproduce, or skipped checklist items
...
- **!!! Don't delete what you didn't create** — flag deletions of unrelated code in the diff.
...
- If the review scope or criteria are unclear, flag it in your output
...
- `agent-browser` — load when reviewing UI changes, verifying visual fidelity, or testing interactive flows (skip if backend-only)
...
- `codebase-design` — load when reviewing module boundaries, seam placement, or interface design
- `review-logging-patterns` — load when reviewing code that adds or modifies logging (skip if no logging changes)
