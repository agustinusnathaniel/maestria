...
- **Guard rails:** run `npx skills --help` before installs (don't memorize flags); install directly, never via `builder`; scan `<available_skills>` for un-prescribed matches and include them. Mid-task skill suggestions: surface via `ask the user`, never install silently. User declines -> spawn anyway; subagent degrades gracefully and flags the missing skill. Never re-ask about the same skill within a task. Subagent can't find a skill -> install reactively and log; repeated misses mean the prescription needs updating.
