...
import { extractModeSection, getModeMarker as sharedGetMarker } from '@maestria/shared-mode';
...
export function getModePrompt(keyword: string): string {
  if ((VALID_KEYWORDS as readonly string[]).includes(keyword)) {
    return MODE_PROMPTS[keyword as ModeKeyword];
  }
  return '';
}
